...
public class PathContainer {        // 包含题目[2]-[9]
    ...
    //@ ensures \result == pList.length; //TODO: [2] ok
    ...
    /*@ ensures \result == (\exists int i; 0 <= i && i < pidList.length; //TODO: [3] ok
      @                      pidList[i] == pathId);
      @*/
    ...
    /*@ public normal_behavior
      ...
      @ also
      @ public exceptional_behavior //TODO: [4] ok
      @ requires !containsPathId(pathId);
      @ assignable \nothing;
    ...
    /*@ public normal_behavior
      ...
      @ requires path == null || !path.isValid() || !containsPath(path); //TODO: [5] ok
    ...
    public /*@ pure @*/ int getPathId(Path path) throws PathNotFoundException {
        if (path != null && path.isValid() && containsPath(path)) {
            ...
            throw new PathNotFoundException(path); //TODO: [6] ok
        }
    }
    ...
    public int addPath(Path path) {
        if (path != null && path.isValid() && !containsPath(path)) {
            ...
            for (Integer node : path) {
                Integer prev = globalNodesCount.get(node);  //TODO: [7] ok
                if (prev == null) {
    ...
    public void removePathById(int pathId) throws PathIdNotFoundException {
        if (containsPathId(pathId)) {
            ...
            for (Integer node : path) {
                ...
                globalNodesCount.put(node, prev - 1);  //TODO: [8] ok
            }
        } else {
            throw new PathIdNotFoundException(pathId);
        }
