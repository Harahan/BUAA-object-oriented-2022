    ...
    for i in range(term_num):
        ...
        for j in range(factor_num):
            ...
            if type == 1:
                ...
                string = generate_function_expr(2, 2, 2, random.randint(0, 100), recursion=recursion-1, alphabet_list=alphabet_list)
                ret_str += generate_space() + string + generate_space()
            else:
                string = generate_pow(alphabet_list)
                ret_str += generate_space() + string + generate_space()
...
if __name__ == '__main__':
    while True:
        test = generate_function_expr(2, 1, 1, random.randint(0, 100), 2)
        print(test)
        std_ans = spy.sympify(test).expand()
        print(std_ans)
        os.chdir("F:\\MyWorkspace\\Object_Oriented_Design\\tools\\excalibur")
        file = open("input.txt", "w")
        file.write('0' + '\n')
        file.write(test)
        file.close()
        os.system("java -jar Archer.jar <input.txt >output.txt")
