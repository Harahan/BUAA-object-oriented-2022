...
"""
def generate_triangle_function(type: bool, alphabet_list=None, recursion=3):
    if alphabet_list is None:
        alphabet_list = ['x']
    choice = (random.randint(0, 100) % 2 == 0)
    if choice is True and recursion != 0:
        if random.randint(0, 100) > 20:
            string, expr = generate_function_expr(1, 1, 1, 2, 1, alphabet_list)
        ...
        if random.randint(0, 100) > 20:
            string, expr = generate_triangle_function(random.choice([True, False]), alphabet_list, recursion=recursion-1)
        else:
...
def generate_custom_function(f_expr, g_expr, h_expr, recursion=1):
    type = random.randint(0, 100)
    x_para_str, x_para_expr = generate_function_call_param(2, 1, 1, random.randint(1, 100), f_expr, g_expr, h_expr, recursion=0, alphabet_list=['x'])
    y_para_str, y_para_expr = generate_function_call_param(1, 1, 1, random.randint(1, 100), f_expr, g_expr, h_expr, recursion=0, alphabet_list=['x'])
    z_para_str, z_para_expr = generate_function_call_param(1, 1, 1, random.randint(1, 100), f_expr, g_expr, h_expr, recursion=0, alphabet_list=['x'])
...
def generate_function_call_param(term_num, factor_num, power_num, symbol_num, f_expr, g_expr, h_expr, recursion=3,
                  alphabet_list=None):
    if alphabet_list is None:
        alphabet_list = ['x']
    ...
    for i in range(term_num):
        ...
        for j in range(factor_num):
            ...
            if type == 1:
                ...
                ret_str += generate_space() + string + generate_space()
                temp_term_expr *= expr
            elif (type >= 5) and recursion > 0:
                string, expr = generate_expr(2, 2, random.randint(0, 8), random.randint(0, 100),
                                             f_expr, g_expr, h_expr, recursion=recursion - 1,
...
if __name__ == '__main__':
    while True:
        ...
        s, e = generate_expr(2, 2, 1, random.randint(0, 100), f_expr, g_expr, h_expr, 2, ['x'])
        print(3)
        ...
        std_ans = e.expand()
        print("Expression OK!")
        os.chdir("Z:\\buaa_oo_2022\\Unit1\\HW3\\expr_test")
