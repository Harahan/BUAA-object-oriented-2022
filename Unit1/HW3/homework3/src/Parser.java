...
public class Parser {
    ...
    public Factor getExpr() {
        lexer.next();
        Expr expr = parseExpr().expand().mergeSquare();
    ...
    public Const getConst() {
        StringBuilder num = new StringBuilder(lexer.peek());
        lexer.next();
    ...
    public Factor getSin() {
        ...
        lexer.next();
        return getSinSignOut(sin);
    }

    public Factor getSinSignOut(Sin sin) {
        Expr expr = new Expr();
        Term term = new Term();
        expr.addTerm(term);
        if (sin.getFactor() instanceof Expr && sin.getFactor().toString().charAt(1) == '-') {
            if (sin.getIndex().mod(BigInteger.valueOf(2)).equals(BigInteger.ONE)) {
                term.addFactor(new Const(BigInteger.valueOf(-1)));
            }
            for (Term item : ((Expr) sin.getFactor()).getTermArrayList()) {
                item.setCoefficient(item.getCoefficient().multiply(BigInteger.valueOf(-1)));
            }
            term.addFactor(sin);
            return expr;
        } else if (sin.getFactor() instanceof Const
                && ((Const) sin.getFactor()).getNum().compareTo(BigInteger.ZERO) < 0) {
            if (sin.getIndex().mod(BigInteger.valueOf(2)).equals(BigInteger.ONE)) {
                term.addFactor(new Const(BigInteger.valueOf(-1)));
            }
            sin.setFactor(new Const(((Const) sin.getFactor()).getNum().abs()));
            term.addFactor(sin);
            return expr;
        }
        return sin;
    ...
    public Factor getCos() {
        ...
        lexer.next();
        return delCosSign(cos);
    }

    public Factor delCosSign(Cos cos) {
        if (cos.getFactor() instanceof Expr && cos.getFactor().toString().charAt(1) == '-') {
            for (Term item : ((Expr) cos.getFactor()).getTermArrayList()) {
                item.setCoefficient(item.getCoefficient().multiply(BigInteger.valueOf(-1)));
            }
        }
        return cos;
