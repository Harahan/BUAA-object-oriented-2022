...
class Model:
	...
	def getClassramdomly(self):
		r = random.randint(0, 100000)
		l = len(self._classesId)
		r = r % l
		return self._classesId[r]
...
class ModelBuilder:
	...
	def get_class_ramdom(self):
		return self._model.getClassramdomly()
	
	# "type":{"$ref":"AAAAAAFqpyZaw1HqYaU="},"direction":"in"}
	def _makeUml(self, id, parent, name, umlType, info):
		...
		if (umlType == self._class):
			...
			if ref == 0:
				...
				uml += stype + ref_start + info[1] + ref_end
			...
			if ref == 0:
				...
				uml += stype + ref_start + info[0] + ref_end
	...
	def createAttribute(self, parentId, attributeName, visibility, attributeType, ref):
		if (parentId not in self._model._classes) & (parentId not in self._model._interfaces):
			return None
		id = self._makeId()
		self._model._addAttribute(parentId, attributeName, visibility, id, attributeType)
		self._makeUml(id, parentId, attributeName, self._attribute, (visibility, attributeType, ref))
		return id
	
	def createParameter(self, parentId, parameterName, parameterType, direction, ref):
		if (parentId not in self._model._operations):
			return None
		id = self._makeId()
		self._model._addParameter(parentId, parameterName, id, parameterType, direction)
		self._makeUml(id, parentId, parameterName, self._parameter, (parameterType, direction, ref))
		return id
