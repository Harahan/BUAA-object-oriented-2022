...
nametype = ['int', 'char', 'byte', 'short', 'long', 'float', 'double', 'boolean', 'void', 'Object']
PARA = 'para'
...
def createParameterRamdomly(builder, opid, parentid, name, mode):
    r = random.randint(0, 1000)
    if r < 500:
        builder.createParameter(opid, name, nametype[r % 10], mode, 0)
    elif 500 <= r and r < 900:
        builder.createParameter(opid, name, '"' + str(builder.get_class_ramdom()) + '"', mode, 1)
    else:
        builder.createParameter(opid, name, '"' + str(parentid) + '"', mode, 1)


def createAttrRamdomly(builder, parentid, parentnum, isClass, attricount):
    r = random.randint(0, 1000)
    if r < 500:
        builder.createAttribute(parentid, makeAttributeName(parentnum, attricount, isClass),
                                makeVisibility(True), nametype[r % 10], 0)
    elif 500 <= r and r < 900:
        builder.createAttribute(parentid, makeAttributeName(parentnum, attricount, isClass),
                                makeVisibility(True), '"' + str(builder.get_class_ramdom()) + '"', 1)
    else:
        builder.createAttribute(parentid, makeAttributeName(parentnum, attricount, isClass),
                                makeVisibility(True), '"' + str(parentid) + '"', 1)
...
def makeOperation(builder, parentid, inputcount, isreturn, visibility, opname):
    ...
    for i in range(inputcount):
        mode = inmode[0]
        createParameterRamdomly(builder, opid, parentid, PARA + str(i), mode)
        # builder.createParameter(opid, PARA + str(i), 'int', mode)
    if (isreturn):
        createParameterRamdomly(builder, opid, parentid, 'return', 'return')
...
def randomParent(total, builder, parentlist, void, create, isClass=True):
    for i in range(total):
        ...
        oppara = []
        if (isClass):
            # print("optotal =" + str(operationtotal))
            ...
            while (attricount < attributetotal):
                ...
                createAttrRamdomly(builder, parentid, parentnum, isClass, attricount)
                # builder.createAttribute(parentid, makeAttributeName(parentnum, attricount, isClass),
                # makeVisibility(True), 'int')
...
class randomUmlMake:
    def __init__(self, filecount):
        file = 'data' + str(filecount) + '.txt'
        filestruct = 'data_struct_' + str(filecount) + '.txt'
